...
const ModalLogin: FC<ModalLoginProps> = ({ isOpen, onOpenChange }) => {
	...
	const { password, login, error, isLoading } = useSelector(getLoginState);
	...
	}, [dispatch, login, onOpenChange, password]);
			...
			>
				<form>
					<Input
						...
						type='text'
						placeholder='Введите login или email'
						variant='bordered'
						onChange={(e) => {
	...
	}, [login, password, onChangeUsername, onChangePassword]);
	...
	return (
		<DynamicModuleLoader reducers={initialReducers}>
			...
			{showNotification && (
				<Notification
					message={`${login}, вы успешно авторизовались!`}
					duration={5000}
					placement='top'
